...
import { LinePayValidationError } from '../src/errors/LinePayError'
...
describe('LinePayUtils', () => {
  ...
  describe('validateTransactionId', () => {
    ...
    test('should throw LinePayValidationError for invalid transaction ID', () => {
      const invalidId = '123456789012345678'
      try {
        LinePayUtils.validateTransactionId(invalidId)
        expect(true).toBe(false) // Should not reach here
      } catch (error) {
        expect(error).toBeInstanceOf(LinePayValidationError)
        if (error instanceof LinePayValidationError) {
          expect(error.message).toContain('Invalid transactionId format')
          expect(error.field).toBe('transactionId')
        }
      }
    })

    test('should throw LinePayValidationError with correct field name', () => {
      try {
        LinePayUtils.validateTransactionId('invalid')
  ...
  describe('generateSignature', () => {
    test('should generate correct signature', () => {
      ...
      const signature = LinePayUtils.generateSignature(channelSecret, uri, body, nonce)
    ...
    test('should generate signature with query string', () => {
      ...
      const signature = LinePayUtils.generateSignature(channelSecret, uri, body, nonce, queryString)
    ...
    test('should generate consistent signature for identical inputs', () => {
      ...
      const signature1 = LinePayUtils.generateSignature(channelSecret, uri, body, nonce)
      const signature2 = LinePayUtils.generateSignature(channelSecret, uri, body, nonce)
  ...
  describe('verifySignature', () => {
    test('should verify signature correctly', () => {
      ...
      const isValid = LinePayUtils.verifySignature(channelSecret, uri, body, nonce, signature)
