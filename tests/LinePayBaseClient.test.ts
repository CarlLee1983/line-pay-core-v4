...
import { LinePayConfigError, LinePayError, LinePayTimeoutError } from '../src/errors/LinePayError'
  ...
  describe('sendRequest', () => {
    ...
    test('should send request with query parameters', async () => {
      const response = await client.testRequest<
        LinePayBaseResponse<{ params: Record<string, string> }>
      >('GET', '/query-test', undefined, {
        foo: 'bar',
        baz: 'qux',
      })
